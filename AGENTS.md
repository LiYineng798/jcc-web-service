...
The `/tools/s16-5-preview` page is the S16.5 champion-only reference surface. Its normalized hero data and local assets live under `static/s16-5-preview/`; keep the 115 heroes, fee groups, trait icons, skill data, and gold icon aligned. The homepage exposes it together with S18 through the desktop `资料库` menu and the grouped mobile resource dialog. The S16.5 page uses the same compact champion filters and skill toggle as S18, and its 14 designated additions carry the `新` badge.
