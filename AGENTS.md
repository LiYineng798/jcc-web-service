...
Season reference data is served from the season library under `static/season-data/` (see `docs/season-library.md`). The library is generated from the external season archive (`ccmax资料/数据模版`) by `scripts/season_library/import_from_archive.py`; never hand-edit `static/season-data/` — fix the archive and re-import instead. `static/season-data/catalog.json` drives the homepage `资料库` menu, the mobile resource dialog, the sitemap, and the `/tools/seasons/<season_id>` pages, so adding a season to the archive plus one import command publishes it everywhere.
...
The admin console uses `templates/admin.html`, `static/admin.js`, `static/admin/core.js`, `static/admin/overview.js`, and the admin-only `static/admin.css`. `admin.js` owns application state, request orchestration, and the remaining workspaces; `admin/core.js` owns shared DOM, escaping, formatting, and debounce helpers; `admin/overview.js` owns overview KPI, traffic chart, todo, and quick-link rendering. Desktop uses a fixed grouped sidebar and compact top bar; mobile uses a dedicated five-item bottom navigation plus a "More" bottom sheet, so all ten modules remain reachable without stacking the desktop navigation. Admin shell icons come from the pinned local Lucide browser build under `static/vendor/lucide/`; do not replace it with a production CDN dependency. Keep the workbench dense, scannable, and operations-focused.
...
Admin startup loads the overview and live-comp season manifest in parallel, and the settings tab loads settings and notices in parallel. The overview's seven-day UV series uses one grouped visit query with zero-filled missing dates; today and yesterday values are derived from that same series.
