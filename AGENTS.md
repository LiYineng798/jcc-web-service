...
The admin console uses `templates/admin.html`, `static/admin.js`, and the admin-only `static/admin.css`. Desktop uses a fixed grouped sidebar and compact top bar; mobile uses a dedicated five-item bottom navigation plus a "More" bottom sheet, so all ten modules remain reachable without stacking the desktop navigation. Admin shell icons come from the pinned local Lucide browser build under `static/vendor/lucide/`; do not replace it with a production CDN dependency. Keep the workbench dense, scannable, and operations-focused.
