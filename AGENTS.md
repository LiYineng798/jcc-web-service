...
The admin lineups tab includes regular-lineup bulk import. Admins paste one lineup-code snippet per line, choose a visible lineup season, and submit to `POST /api/admin/lineups/bulk-import`. The importer extracts the lineup name from the first hash-delimited segment, using the text after the final hyphen when present, writes new rows as the current admin user, and skips both database-existing codes and duplicates within the same upload.
