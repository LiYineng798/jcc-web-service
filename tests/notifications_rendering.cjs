...
(async () => {
 ...
 for (const [name,engine] of [['edge',chromium],['webkit',webkit]]) {
  ...
  await admin.locator('.admin-sidebar [data-admin-tab="season-display"]').click();
  await admin.locator('[data-season-surface="simulator"] .season-display-status').first().selectOption('archived');
  ...
  await admin.locator('[data-season-surface="simulator"] .season-display-status').first().selectOption('active');
