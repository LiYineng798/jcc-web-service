...
(async () => {
  for (const [name, engine] of [['chromium', chromium], ['webkit', webkit]]) {
    ...
    try {
      ...
      assert.equal(await page.locator('.toolbox-panel a').first().getAttribute('class'), await page.locator('.toolbox-panel a').nth(1).getAttribute('class'));
      ...
      await page.locator('.opening-copy').first().click();
      assert(await page.getByText('复制失败，请允许浏览器访问剪贴板后重试', { exact: true }).isVisible());
      assert.equal(await page.locator('.opening-copy').first().isDisabled(), false);
      ...
      assert.equal(await page.locator('#mobileResourceDialog a[href="' + route + '"]').getAttribute('class'), await page.locator('#mobileResourceDialog a[href="/tools/special-mechanics"]').getAttribute('class'));
      ...
      assert.equal(await staticPage.locator('textarea, .opening-manual').count(), 0);
      await noJS.close();
