...
(async () => {
  ...
  for (const [name, engine] of [['chromium', chromium], ['webkit', webkit]]) {
    ...
    try {
      ...
      await lib.locator('.season-display-public > li').first().waitFor();
      assert.deepEqual(await publicIds(lib), ['s16_5','s18']);
      const dataCatalogBefore = await api('library');
      await page.locator('.admin-sidebar [data-admin-nav-toggle="live-comps"]').click();
      await page.locator('.admin-sidebar [data-admin-workspace="seasons"]').click();
      const live = surface('live');
      await live.locator('.season-display-public > li').first().waitFor();
      await settle();
      const liveIds = ['s18','s16-5-legends','s8-monsters-attack'];
      assert.deepEqual(await publicIds(live), liveIds);
      assert.deepEqual(await live.locator('.season-display-rank').allTextContents(), ['1','2','3']);
      assert.equal(await live.locator('.season-display-inactive .season-display-arrow').count(),0);
      for (const theme of ['light','dark']) {
        if (await page.evaluate(() => document.documentElement.dataset.theme) !== theme) await page.locator('#themeToggle').click();
        await page.waitForFunction(value => getComputedStyle(document.body).color === (value === 'dark' ? 'rgb(243, 238, 230)' : 'rgb(33, 31, 28)'), theme);
        for (const width of [1440,1024,768,390,320]) {
          await page.setViewportSize({width,height:1000});
          assert(await page.evaluate(() => document.documentElement.scrollWidth<=innerWidth), `live ${name} ${theme} ${width} overflow`);
          const bounds = await page.locator('.live-season-manager button,.live-season-manager select,.live-season-manager summary').evaluateAll(nodes => nodes.filter(n=>n.getClientRects().length).map(n=>{
            const r=n.getBoundingClientRect();return {left:r.left,right:r.right,height:r.height};
          }));
          assert(bounds.every(r=>r.left>=0 && r.right<=width+1), `live ${name} ${width} controls overflow`);
          if(width<=520) assert(bounds.every(r=>r.height>=44), `live ${name} ${width} touch targets`);
          if ([1440,390].includes(width)) await page.screenshot({path:`${out}/${name}-live-${theme}-${width}.png`,fullPage:width>520});
        }
      }
      await page.setViewportSize({width:1440,height:1000});
      await live.locator('summary').click();
      await page.screenshot({path:`${out}/${name}-live-expanded.png`,fullPage:true});
      await live.getByRole('button',{name:'下移S18 · 自然之力',exact:true}).click();
      await settle();
      assert.deepEqual(await publicIds(live), ['s16-5-legends','s18','s8-monsters-attack']);
      assert.deepEqual((await (await auth.get('/api/lineup-seasons')).json()).seasons.map(s=>s.id),await publicIds(live));
      await live.getByRole('button',{name:'重新展示S17 · 星神',exact:true}).click();
      await settle();
      assert.deepEqual(await publicIds(live), ['s16-5-legends','s18','s8-monsters-attack','s17-star-god']);
      await status(live,'s17-star-god','disabled');
      await status(live,'s18','hidden');
      const liveApi = async () => (await auth.get('/api/live-comps/seasons')).json();
      assert.equal((await liveApi()).default_season_id,'s16-5-legends');
      await live.getByRole('button',{name:'将S8·怪兽入侵设为实时阵容默认赛季',exact:true}).click();
      await settle();
      assert.equal((await liveApi()).default_season_id,'s8-monsters-attack');

      // Creation uses its mutation response, even if a follow-up listing would fail.
      await page.route('**/api/admin/live-comps/seasons', route => route.request().method()==='GET'
        ? route.fulfill({status:503,contentType:'application/json',body:'{"error":"演练：刷新失败"}'}) : route.continue());
      ...
      await page.locator('#liveSeasonIdInput').fill('preview-new');
      await page.locator('#liveSeasonNameInput').fill('验收新增赛季');
      await page.getByRole('button',{name:'创建赛季',exact:true}).click();
      await page.locator('#liveSeasonIdInput').waitFor({state:'detached'});
      await live.locator('.season-display-inactive [data-season-id="preview-new"]').waitFor();
      assert(await live.locator('details').evaluate(n=>n.open));
      assert(!(await liveApi()).seasons.some(s=>s.id==='preview-new'));
      await page.unroute('**/api/admin/live-comps/seasons');
      await live.getByRole('button',{name:'重新展示验收新增赛季',exact:true}).click();
      await settle();
      assert.equal((await publicIds(live)).at(-1),'preview-new');
      assert.equal((await (await auth.get('/api/lineup-seasons')).json()).seasons.at(-1).id,'preview-new');
      await page.route('**/api/admin/live-comps/seasons/preview-new', route => route.fulfill({status:400,contentType:'application/json',body:'{"error":"演练：实时赛季保存失败"}'}));
      await status(live,'preview-new','disabled');
      await page.getByText('演练：实时赛季保存失败',{exact:true}).waitFor();
      assert.equal(await live.locator('[data-season-id="preview-new"] select').inputValue(),'active');
      await page.unroute('**/api/admin/live-comps/seasons/preview-new');
      for (const id of await publicIds(live)) await status(live,id,'disabled');
      assert.deepEqual(await liveApi(),{default_season_id:null,seasons:[]});
      await live.getByText('暂无展示中的赛季，可从下方重新展示。',{exact:true}).waitFor();
      await live.getByRole('button',{name:'重新展示S18 · 自然之力',exact:true}).click();
      await settle();
      assert.equal((await liveApi()).default_season_id,'s18');
      assert.deepEqual(await api('library'),dataCatalogBefore);
      await page.getByRole('button',{name:'刷新赛季',exact:true}).click();
      await settle();
      assert.deepEqual(await publicIds(live),['s18']);
      await page.setViewportSize({width:390,height:1000});
      await page.locator('#adminMoreButton').click();
      await page.locator('#adminMoreDialog [data-admin-workspace="seasons"]').click();
      await settle();
      assert.equal(await page.locator('#adminMoreDialog').evaluate(n=>n.open),false);
      assert.deepEqual(await publicIds(live),['s18']);
      assert.deepEqual(errors, []);
      console.log(`${name}: library/simulator/live season layout, ordering, restore, default, creation, persistence and failure checks passed`);
    } finally {
      await auth?.dispose();
      await browser?.close();
      await server.close();
