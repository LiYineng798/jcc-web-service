...
(async () => {
  ...
  for (const [engineName, engine] of [['chromium', chromium], ['webkit', webkit]]) {
    ...
    await notice.getByRole('button', { name: '归档', exact: true }).click();
    await notice.waitFor({ state: 'detached' });
    await accountPage.getByRole('button', { name: '已归档', exact: true }).click();
    await notice.waitFor(); await notice.getByRole('button', { name: '移出归档', exact: true }).click();
    await accountPage.getByRole('button', { name: '未归档', exact: true }).click();
    await notice.getByRole('link', { name: '修改并提交重审' }).click();
    ...
    await browser.close(); console.log(engineName + ': responsive themes, accessible modal, read/archive, ban/reject/resubmit/approve passed');
  }
