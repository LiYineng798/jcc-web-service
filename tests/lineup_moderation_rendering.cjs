...
(async () => {
  ...
  for (const [engineName, engine] of [['chromium', chromium], ['webkit', webkit]]) {
    ...
    await accountPage.goto(`${base}/?notifications=open`);
    await accountPage.locator('.ln-item').first().waitFor();
    for (const theme of ['light', 'dark']) for (const width of [320, 390, 768, 1440]) {
      for (const [surface, tab] of [['admin', page], ['account', accountPage]]) {
        ...
        if (surface === 'account') {
          const bounds = await tab.locator('.ln-panel').boundingBox();
          assert(bounds.x >= 0 && bounds.x + bounds.width <= width && bounds.y + bounds.height <= 1000, 'popover outside viewport');
        }
    ...
    await accountPage.goto(`${base}/?notifications=open`);
    const notice = accountPage.locator(`.ln-item[data-lineup-id="${lineup.id}"]`); await notice.waitFor();
    assert(await notice.getByText(reason, { exact: true }).isVisible());
    assert.equal(await notice.locator('.ln-preview img').count(), 0);
    ...
    await accountPage.waitForURL(`**/me/lineup-notifications/${lineup.id}`);
    await accountPage.locator('#lineupNotificationDetail[aria-busy="false"]').waitFor();
    assert.equal((await (await owner.request.get(`${base}/api/lineups/${lineup.id}/moderation`)).json()).moderation.notice_state, 'read');
    await accountPage.getByRole('link', { name: '修改重审', exact: true }).click();
    ...
    await accountPage.waitForURL(`**/me/lineup-notifications/${lineup.id}`);
    await accountPage.getByText('待审核', { exact: true }).waitFor();
    ...
    await page.locator('.lm-dialog').waitFor({ state: 'detached' });
    await accountPage.reload(); await accountPage.getByText('已退回', { exact: true }).waitFor();
    await accountPage.getByRole('link', { name: '修改重审', exact: true }).click();
    await accountPage.getByRole('heading', { name: '修改并提交重审', exact: true }).waitFor();
    ...
    await accountPage.reload(); await accountPage.locator('.lm-badge.lm-approved').waitFor();
