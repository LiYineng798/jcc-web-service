...
(async()=>{
 for(const [name,engine] of [['chromium',chromium],['webkit',webkit]]){
  ...
  try{
   ...
   assert.equal(await page.locator('#themeToggle').getAttribute('aria-label'),'切换为白天模式');
   await page.reload();
   await page.locator('[data-hero]').last().waitFor();
   assert.equal(await page.locator('html').getAttribute('data-theme'),'dark');
   assert((await page.locator('#themeToggle').getAttribute('class')).includes('is-dark'));
   ...
   assert.equal(await page.locator('.selected-hero').count(),0);
   ...
   assert.equal(await page.locator('.trait-chip').first().evaluate(e=>getComputedStyle(e).backgroundColor),'rgb(245, 239, 229)');
   assert((await page.locator('.trait-chip img').first().evaluate(e=>getComputedStyle(e).filter)).includes('brightness(0)'));
   await page.locator('.result-card').first().screenshot({path:`${output}/${name}-trait-light.png`});
   await page.locator('#themeToggle').click();
   await page.waitForTimeout(250);
   await page.locator('.result-card').first().screenshot({path:`${output}/${name}-trait-dark.png`});
   await page.locator('#themeToggle').click();
   await page.waitForTimeout(250);
   ...
   assert.equal(await page.locator('[data-emblem="341"]').getAttribute('class'),'is-selected');
   assert.equal(await page.locator('[data-emblem="341"] b').textContent(),'1');
   assert.equal(await page.locator('.selected-hero').count(),2);
   ...
   await page.locator('#lux').selectOption('455'); await page.locator('[data-khazix="349"]').click();
   await page.locator('[data-emblem="341"]').click();
   await page.locator('[data-clear-selected]').click();
   assert.equal(await page.locator('#lux').inputValue(),'');
   assert.equal(await page.locator('#khazix-choices [aria-pressed="true"]').count(),0);
   assert.equal(await page.locator('[data-emblem="341"] b').textContent(),'1');
   await page.locator('#reset').click();
