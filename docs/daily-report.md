...
| `summary.returning_3d / returning_7d` | 昨日访客中，近 3 日 / 近 7 日（不含昨日）内也访问过的人数 |
...
| `season_copy_rank` | 版本复制榜：按 `season_id` 聚合的复制次数、人数与占比 |
| `top_visitor_ips` | 访问 IP Top：次数、人数、页面数、是否回访 IP、当日是否有复制（仅管理员可见） |

口径说明：

- 访问统计排除管理员与机器人 UA（与 `visits.py` 一致）。
- 复制统计以 `copy_action_events` 中 `success = 1` 的行为为准（与后台「今日复制排行」同源）。
- 回访口径：`returning_3d/7d` 统计昨日访客中在 `[日期-3/7, 日期)` 窗口内出现过
  `visit_events` 的人数，比"曾经来过"的新老访客更贴近近期留存。
- 访问 IP 面板为运维辅助视图（IP 可能被 NAT/共享网络复用），仅管理员可见，
  与后台复制明细中的 IP 展示口径一致。
