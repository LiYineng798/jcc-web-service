...
本次修改把 `/me` 从头像、概览、两段历史、反馈、阵容依次堆叠的长页面，改为可切换的个人空间。代码位于 Web 服务的 `codex/profile-workspace` 分支；验收前不合并 main。公开作者页 `/author/<username>` 继续作为对外展示入口。
...
- **概览下半部分**：展示最近三条浏览记录和快捷复制；创作备忘显示隐藏阵容、自己阵容收到的待处理反馈、自己提交的反馈，并链接到相应工作区及原通知入口。
- **我的阵容、收藏**：使用原 `/api/lineups` 的 `view`、`q`、`page`、`page_size=4`，支持完整分页及搜索，不再只展示前 20 条。桌面采用两列、每页四条，使常规笔记本尺寸下的分页按钮也处于首屏。收藏提供复制和取消收藏；我的阵容提供编辑，封禁项跳转到已有封禁与重审详情。
...
首次只加载当前登录用户、概览与最近浏览。其他栏目按需请求；切换栏目或离开页面会取消旧请求，避免迟到响应覆盖新内容。请求错误有重试，登录失效提供重新登录入口；不把失败数据伪装成零值。现有登录、CSRF、收藏/复制权限、封禁重审和反馈 API 保持原实现。
...
本地验证记录（2026-09-14）：`python -m pytest -q` 全量 **666 passed**；`npm run build --prefix frontend` 三个组件构建成功；`node tests/account_profile_rendering.cjs` 的 Chromium/Edge 和 WebKit 均通过上述检查。后台已有静态 bundle 重建后无内容变化。

本次只有 Web 仓库变更。无需 PostgreSQL 迁移、环境变量或额外 worker。保留用户验收前的 worktree 预览；确认后再合并 main。未来发布沿用仓库既有备份、静态资源权限修复、健康检查流程。
